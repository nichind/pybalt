...
class Remuxer:
    ...
    def remux(cls, path: Path | str) -> Path:
        ...
        try:
            Popen(["ffmpeg", "-hwaccel", "opencl", "-i", str(path), "-c", "copy", str(output), "-progress", str(progress_file), "-loglevel", "error"], stdout=None, stderr=None, stdin=None)
            last_update = 0
            data = {}
            while True:
                sleep(0.5)
                ...
                if data.get("progress", "") == "end":
                    break
                print(f'Remuxing status: {data.get("progress", "unknown")}, speed: {data.get("speed", "0.00x")}, fps: {data.get("fps", "0.00")}, frame: {data.get("frame", "0")}', end='\r')
