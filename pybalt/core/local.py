...
class LocalInstance:
    ...
    def generate_docker_compose_file(self) -> None:
        ...
        if api_auth_required == "1" and api_key:
            ...
            compose_data["services"]["cobalt-api"]["volumes"].append(f"./keys.json:/keys.json")
...
"""
        ...
        port = click.prompt("Enter port number for the instance", default=9001, type=int)
        ...
        duration_limit = click.prompt("Enter maximum duration limit in seconds (e.g., 10800 = 3 hours)", 
                                     default=10800, type=int)
        
        disabled_services = click.prompt("Enter comma-separated list of services to disable (leave empty for none)", 
                                        default="")
...
def config(port, auth, api_key, duration_limit, disabled_services):
    ...
    if changed:
        ...
        click.echo(f"Port: {config.get_as_number('local_instance_port', 9001, 'local')}")
...
def update():
    ...
    try:
        ...
        subprocess.run(
            compose_cmd + ["-f", str(local_instance.docker_compose_path), "up", "-d", "--force-recreate"],
            check=True,
            cwd=local_instance.instance_dir
        )
