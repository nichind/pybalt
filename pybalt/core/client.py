...
from .config import get_float_value as gfv
...
class HttpClient:
    ...
    def __init__(
        ...
        if auto_detect_proxy and not proxy:
    ...
    async def request(
        ...
        verify: bool = None,
        retries: int = 0,
        max_retries: int = gfv("MAX_RETRIES", 5),
        close: bool = True,
        **kwargs,
    ) -> Response:
        ...
        request_timeout = ClientTimeout(
            total=timeout or self.timeout or gfv("TIMEOUT", 30)
        )
            ...
            try:
                ...
                async with session_method(full_url, **session_kwargs) as response:
                    ...
                    if response.status == 429:
                        retry_after = float(
                            response.headers.get("Retry-After", gfv("RETRY_DELAY", 1))
                        )
    ...
    async def download_file(
        ...
        **options: Unpack[DownloadOptions],
        ...
        callback_rate = options.get("callback_rate", gfv("CALLBACK_RATE", 0.256))
        ...
        request_timeout = ClientTimeout(
            total=options.get("timeout", self.timeout or gfv("TIMEOUT", 30))
        )
